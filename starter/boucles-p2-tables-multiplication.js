...
let table = parseInt(prompt("Je peux vous afficher les n premières tables de multiplication. \nDites-moi à laquelle vous souhaitez que je m'arrête. \nPar exemple, si vous entrez 4, je vous afficherai les tables de multiplication par 1, par 2, par 3 et par 4. \nJusqu'à quelle table voulez-vous que j'aille ?"));
const nbrTable = parseInt(prompt("Combien de valeurs souhaitez-vous que j'affiche pour chacune de ces tables ?"))

for (let i = 1; i <= table; i++) {
    console.log(`Voici les ${nbrTable} premières valeurs de la table de multiplication par ${i}`);
    for (let y = i; y <= i * nbrTable; y += i) {
        console.log(y);
    }
}
